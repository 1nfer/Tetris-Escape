...
public class HandFistMove : MonoBehaviour
{
    [Header("Movement Settings")]
    public float moveSpeed = 2.0f;
    public float rayDistance = 1.5f; // Reducido para detección más precisa
    public bool useRightHand = true;
    
    [Header("Obstacle Detection Settings")]
    public float maxAutoJumpHeight = 2f;
    public float detectionRadius = 0.4f; // Ajustado al 80% del radius del CharacterController
    ...
    public float raySpread = 0.6f; // Aumentado para cubrir todo el ancho del player
    ...
    void Start()
    {
        ...
        if (handSubsystem == null)
            Debug.LogError("❌ No se encontró XRHandSubsystem.");
    }
    ...
    {
        Debug.Log("🏆 Jugador VR alcanzó el objetivo! Victoria!");
    ...
    void TryMoveForward()
    {
        ...
        // CORRECCIÓN CLAVE: Calcular la base real del CharacterController
        float playerBaseY = transform.position.y - (controller.height * 0.5f);

        // DETECCIÓN MEJORADA CON MÚLTIPLES SPHERECASTS
        if (useMultipleRays)
        {
            // Alturas de detección ajustadas a la geometría real del capsule
            float[] heightOffsets = new float[] 
            { 
                controller.height * 0.25f,  // Bajo
                controller.height * 0.5f,   // Medio
                controller.height * 0.75f   // Alto
            };
        ...
        // Auto-salto si se detecta obstáculo saltable
    ...
    void ProcessObstacleHit(RaycastHit hit, Vector3 moveDirection, float playerBaseY, ref bool obstacleDetected, ref float obstacleHeight, ref float closestDistance)
    {
        ...
        // CORRECCIÓN CLAVE: Cálculo preciso de altura del obstáculo
        Bounds obstacleBounds = hit.collider.bounds;
        float obstacleTop = obstacleBounds.max.y;
        float obstacleBottom = obstacleBounds.min.y;
        
        // Altura del obstáculo desde la base del player
        float heightDifference = obstacleTop - playerBaseY;
        
        // Verificar que el obstáculo esté en el camino vertical del jugador
    ...
    void OnDrawGizmosSelected()
    {
        ...
        // Visualizar múltiples alturas de detección
